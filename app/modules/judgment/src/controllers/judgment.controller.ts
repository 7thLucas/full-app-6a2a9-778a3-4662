...
import {
  ...
  listTasks,
  listSubmissions,
} from "../services/judgment-overview.service";
...
export async function getOverview(req: Request, res: Response) {
  try {
    return res.json(await getComplianceOverview());
  } catch (err: any) {
...
export async function getIssues(req: Request, res: Response) {
  try {
    return res.json(await listIssues());
  } catch (err: any) {
    logger.error("getIssues failed", err);
    return res.status(500).json({ error: err.message });
  }
}
...
export async function getTasks(req: Request, res: Response) {
  try {
    return res.json(await listTasks());
  } catch (err: any) {
